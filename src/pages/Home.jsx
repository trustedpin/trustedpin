  ...
  return (
    <div style={{background:'#08080E',minHeight:'100vh'}}>
      ...
      <div style={{background:'#0F0F1A',padding:'3rem 1.25rem 2.5rem',textAlign:'center',position:'relative',overflow:'hidden'}}>
        <div style={{position:'absolute',top:0,left:'50%',transform:'translateX(-50%)',width:'100%',height:'1px',background:'linear-gradient(90deg,transparent,#C9A84C,transparent)',opacity:0.4}}></div>
        ...
        <p style={{color:'#C8C0B4',fontSize:'0.95rem',maxWidth:'480px',margin:'0 auto 2rem',lineHeight:'1.75'}}>
          We search Google so you don't have to. Enter your ZIP code, pick a service, and instantly see the best-reviewed local contractors — phone number, hours, and reviews included.
        </p>

        <div style={{background:'#181828',border:'1px solid #252538',borderRadius:'16px',padding:'1.25rem',maxWidth:'560px',margin:'0 auto',boxShadow:'0 20px 50px rgba(0,0,0,0.4)'}}>
          <p style={{fontSize:'0.82rem',fontWeight:'600',color:'#F2EEE6',marginBottom:'1rem',textAlign:'left'}}>What do you need help with today?</p>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
            <div>
              ...
              <select value={service} onChange={e=>setService(e.target.value)} style={{width:'100%',background:'#212135',border:'1px solid #252538',borderRadius:'9px',padding:'0.8rem 0.9rem',fontSize:'1rem',fontFamily:'DM Sans,sans-serif',color: service ? '#F2EEE6' : '#6A6258',outline:'none'}}>
            ...
            <div>
              ...
              <input value={location} onChange={e=>setLocation(e.target.value)} onKeyDown={e=>e.key==='Enter'&&search()} placeholder='e.g. Detroit, MI or 48075' style={{width:'100%',background:'#212135',border:'1px solid #252538',borderRadius:'9px',padding:'0.8rem 0.9rem',fontSize:'1rem',fontFamily:'DM Sans,sans-serif',color:'#F2EEE6',outline:'none'}} />
            </div>
            <button onClick={()=>search()} style={{width:'100%',background:'#C9A84C',color:'#08080E',border:'none',borderRadius:'9px',padding:'0.85rem',fontSize:'1rem',fontWeight:'700',cursor:'pointer',fontFamily:'DM Sans,sans-serif',transition:'all 0.2s'}}>
              Find Pros Near Me →
            </button>
          </div>

          <div style={{marginTop:'1rem',borderTop:'1px solid #252538',paddingTop:'1rem'}}>
            <p style={{fontSize:'0.72rem',color:'#9A8870',marginBottom:'0.6rem',textAlign:'left'}}>Quick search:</p>
            <div style={{display:'flex',flexWrap:'wrap',gap:'7px'}}>
              {[{name:'Plumbing',icon:'🔧'},{name:'Electrical',icon:'⚡'},{name:'HVAC',icon:'❄️'},{name:'Roofing',icon:'🏠'},{name:'Cleaning',icon:'🧹'},{name:'Handyman',icon:'🛠️'}].map(c=>(
                <button key={c.name} onClick={()=>{setService(c.name);if(location.trim())search(c.name,location.trim())}} style={{background:'#252535',border:'1px solid #2A2A3A',borderRadius:'100px',padding:'0.38rem 0.9rem',fontSize:'0.8rem',color:service===c.name?'#C9A84C':'#C8C0B4',cursor:'pointer',fontFamily:'DM Sans,sans-serif',border:service===c.name?'1px solid #C9A84C':'1px solid #2A2A3A',transition:'all 0.15s'}}>
                  {c.icon} {c.name}
                </button>
              ))}
            </div>
      ...
      <div style={{background:'#181828',borderTop:'1px solid #252538',borderBottom:'1px solid #252538',padding:'1rem 1.25rem',display:'flex',justifyContent:'center',gap:'1.5rem',flexWrap:'wrap'}}>
